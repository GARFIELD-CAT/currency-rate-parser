...
Я использую **Micrometer** для сбора и мониторинга производительности приложения. Основные метрики, которые я собираю:
...
### Настройка VisualVM
Я установил **VisualVM** для мониторинга моего Java-приложения и сбора данных о производительности.

### Поиск узких мест
- **Медленные методы**: Я исследовал методы с длительным временем выполнения, чтобы определить узкие места.
...
- **Thread Dumps**: Я использовал для анализа состояния потоков в приложении. Явных проблем с потоками не было обнаружено.
![img_3.png](img_3.png)

- **Heap Dumps**: Я изучал активные объекты, чтобы выяснить, какие из них занимаются больше всего памяти. Чвных проблем с потреблением памяти не было обнаружено. Память эффективно очищается.
![img_2.png](img_2.png)
![img_4.png](img_4.png)
